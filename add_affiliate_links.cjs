...
const USE_SHORT_LINKS = process.env.USE_SHORT_LINKS === 'true'; // Default to false
...
class iTunesAffiliateLinkGeneratorWithShortener {
    ...
    extractSongMentions(content) {
        const songRegex = /\[SONG:\s*"([^"]+)"\s*by\s*"([^"]+)"\]/g;
        const songs = [];
        let match;
        ...
        try {
            ...
            for (const song of songs) {
                ...
                try {
                    ...
                    const enhancedMention = `[${song.title} by ${song.artist}](${finalLink})`;
